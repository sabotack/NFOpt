...
def process_flows_hour(timestamp, flows, traffic, args, linksCopy):
    ...
    for flow in flows:
        ...
        for linkKey in flowLinks:
            if(linkKey in links):
                links[linkKey]['totalTraffic'] += traffic[flow] * flowLinks[linkKey].trafficRatio
            else:
                links[linkKey] = {
    ...
    if (args.model_type == CalcType.BASELINE.value):
        ...
        return [timestamp, min(linkUtil.values()), max(linkUtil.values()), stats.mean(linkUtil.values())] 
    else:
        avgLinkUtil, minLinkUtil, maxLinkUtil = linOpt.runLinearOptimizationModel(args.model_type, links, flows, traffic, timestamp, args.save_lp_models)
...
def main():
    ...
    parser.add_argument('-slpm', '--save-lp-models', action='store_true', help='save linear optimization models')
    ...
    with mp.Pool() as pool:
        results = pool.starmap(process_flows_hour, [(timestamp, flows[timestamp], traffic[timestamp], args, links.copy()) for timestamp in flows])
    
    logger.info("Finished processing all timestamps!")

    results.sort()
    dataUtils.writeDataToFile(pd.DataFrame(results, columns=['timestamp', 'min_util', 'max_util', 'avg_util']), args.model_type)

    endTime = pd.Timestamp.now()

    # Log elapsed time in hours, minutes and seconds
    elapsedTime = (endTime - startTime).components
    logger.info(f'Finished, elapsed time: {elapsedTime.hours} hours, {elapsedTime.minutes} minutes, {elapsedTime.seconds} seconds')
