...
def optMC(parserArgs, links, flowTraffic, flows, timestamp):
    ...
    if m.Status == gp.GRB.OPTIMAL:
        ...
        for flow, value in flow_values.items():
            logger.info(f"{flow}: {value}")


        #print utilization
        for link in utilization:
            logger.info(f"{link}: {utilization[link].X}")
        ...
        pdC = pd.DataFrame(all_paths_with_ratios, columns=["timestamp", "flowName", "path", "ratio"],),
...
optMC(None, None, None, None, None)